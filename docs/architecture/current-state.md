...
- La confianza "Alta" depende de las pruebas disponibles en el repo: 628 tests unitarios/mockeados en el commit auditado (634 a la fecha de este párrafo, sin tocar código de producción), más, desde el 2026-07-25, 34 tests de integración contra `wacrm-staging` que cubren las cinco RPCs de aislamiento y `requireApiKey` (ver fichas 1.3, 1.5, 2.13). Fuera de esas seis piezas, la ausencia de prueba contra base real (ninguna de los 11 handlers HTTP de `/api/v1/*`, ninguna del resto de rutas y libraries de la sección 2) sigue limitando la certeza a lectura estática, marcada explícitamente como confianza Media/Baja en esas fichas. La suite de integración reveló además un hallazgo propio (sección 3): el guard anti-producción de `tests/integration/setup/guard-staging.ts` no cubre el cliente interno que usa `requireApiKey` (`supabaseAdmin()` de `@/lib/flows/admin-client`), que en este repo lee variables de entorno apuntadas a producción.
